 ...
 * (isSubmission: true). Editors review + publish them from the
 * "📥 Приказни за преглед" queue in Studio. No Supabase, no email.
 *
...
// Best-effort in-memory rate limit (per server instance). For hardened limits
// across serverless instances, front this with Turnstile or Upstash later.
...
export async function POST(req: Request) {
  try {
    ...
    if ((form.get("website") as string)?.trim()) {
      return NextResponse.json({ ok: true }); // pretend success, create nothing
    }
    ...
    for (const file of imageFiles.slice(0, MAX_IMAGES)) {
      ...
      // Reject oversized files and non-images — protects asset quota
    ...
    const doc = {
      ...
      ...(subject       && { subject }),
      ...(videoUrl      && { videoUrl }),
      // Always record the authenticated submitter (form fields are optional overrides)
      submittedBy: {
    ...
    // Create as a draft so it never appears publicly until an editor publishes it
