...
import { saveMembershipTier } from "../../app/actions/membership";
...
const EMPTY_MEMBER: MemberForm = {
  name: "", email: "", phone: "", message: "", membership: "volunteer",
};
const EMPTY_COMPANY: CompanyForm = {
  company: "", contact: "", email: "", phone: "", message: "", tier: "basic",
};
...
const COMPANY_TIERS: { value: CompanyTier; label: string; price: string; perks: string[] }[] = [
  {
    ...
    label: "Основно",
    price: "5.000 ден / год",
    perks: ["Лого во апликацијата", "Благодарница на социјалните мрежи"],
  },
  {
    value: "preferred",
    label: "Преферирано",
    price: "15.000 ден / год",
    perks: ["Сè од Основно", "Истакнато место на страницата", "Месечен извештај"],
  },
  {
    value: "premium",
    label: "Премиум",
    price: "По договор",
    perks: ["Сè од Преферирано", "Брендирани акции", "Партнерски настани"],
  },
];

interface Props {
  onClose: () => void;
  /** Current logged-in user ID — if provided, tier is saved to their profile */
  userId?: string | null;
...
export default function PartnerModal({ onClose, userId, prefillName, prefillEmail }: Props) {
  ...
  function back() { setStep("choose"); setDone(false); }

  function buildMemberBody() {
    const tier = MEMBERSHIP_OPTIONS.find((o) => o.value === memberForm.membership)!;
    return (
      `Полно Име: ${memberForm.name}\n` +
      `Е-пошта: ${memberForm.email}\n` +
      `Телефон: ${memberForm.phone || "—"}\n` +
      `Членарина: ${tier.label} (${tier.price})\n\n` +
      `Порака:\n${memberForm.message || "—"}`
    );
  }

  function buildCompanyBody() {
    const tier = COMPANY_TIERS.find((t) => t.value === companyForm.tier)!;
    return (
      `Компанија: ${companyForm.company}\n` +
      `Контакт лице: ${companyForm.contact || "—"}\n` +
      `Е-пошта: ${companyForm.email}\n` +
      `Телефон: ${companyForm.phone || "—"}\n` +
      `Пакет: ${tier.label} (${tier.price})\n\n` +
      `Порака:\n${companyForm.message || "—"}`
    );
  }
  ...
  async function submitMember(e: React.FormEvent) {
    ...
    setSubmitting(true);
    // Map local tier to DB enum value
    const dbTier = memberForm.membership as "volunteer" | "monthly" | "yearly";
    if (userId) await saveMembershipTier(dbTier);
    const subject = encodeURIComponent("Станувам член — Мој Прилеп");
    window.open(`mailto:mojpprilep@gmail.com?subject=${subject}&body=${encodeURIComponent(buildMemberBody())}`);
    setSubmitting(false);
    setDone(true);
    toast.success("Ви благодариме!");
  }
  ...
  async function submitCompany(e: React.FormEvent) {
    ...
    setSubmitting(true);
    const dbTier = `company_${companyForm.tier}` as "company_basic" | "company_preferred" | "company_premium";
    if (userId) await saveMembershipTier(dbTier);
    const subject = encodeURIComponent("Компанија партнер — Мој Прилеп");
    window.open(`mailto:mojpprilep@gmail.com?subject=${subject}&body=${encodeURIComponent(buildCompanyBody())}`);
    setSubmitting(false);
    setDone(true);
    toast.success("Ви благодариме!");
  }
  ...
  const modal = (
    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center sm:p-4" role="dialog" aria-modal="true">
      ...
      {/* Sheet — fullscreen on mobile, max-w-lg centered on desktop */}
      <div className="relative flex w-full flex-col overflow-hidden bg-white shadow-2xl
                      h-[90dvh] rounded-t-2xl
                      sm:h-auto sm:max-h-[88dvh] sm:max-w-lg sm:rounded-2xl">
        ...
        <div className="shrink-0 flex items-center gap-2 border-b border-zinc-100 px-5 py-4">
          {step !== "choose" && !done ? (
            ...
            <div className="h-8 w-8 shrink-0 hidden" />
          )}
          ...
          {stepTitle && (
            <span className="text-sm font-semibold text-zinc-500">{stepTitle}</span>
          )}
        ...
        <div className="flex-1 min-h-0 overflow-y-auto">
          ...
          {step === "choose" && (
            <div className="px-5 py-6 space-y-3">
              ...
              <ChoiceCard
                icon={<User size={22} />}
                title="Станете член"
                desc="Поединци кои сакаат да придонесат со своето време, знаење или членарина."
                onClick={() => setStep("member")}
              />
              <ChoiceCard
                icon={<Building2 size={22} />}
                title="Компанија партнер"
                desc="Бизниси кои сакаат да вложат во заедницата и да добијат видливост."
          ...
          {step === "member" && !done && (
            <form id="member-form" onSubmit={submitMember} className="px-5 py-5 space-y-5">
              <Field label="Полно Име *" icon={<User size={14} />} placeholder="Вашето Име и Презиме"
                value={memberForm.name} onChange={(v) => setMemberForm((f) => ({ ...f, name: v }))} required />
              <Field label="Е-пошта *" icon={<Mail size={14} />} type="email" placeholder="вашата@епошта.com"
                value={memberForm.email} onChange={(v) => setMemberForm((f) => ({ ...f, email: v }))} required />
              <Field label="Телефон" icon={<Phone size={14} />} type="tel" placeholder="+389 7X XXX XXX"
                value={memberForm.phone} onChange={(v) => setMemberForm((f) => ({ ...f, phone: v }))} />
                  ...
                  {MEMBERSHIP_OPTIONS.map((opt) => (
                    <button
                      ...
                      className={cn(
                        ...
                        memberForm.membership === opt.value
              ...
              <div>
                ...
                <textarea rows={3} placeholder="Вашите вештини, идеи, прашања…" lang="mk"
          ...
          {step === "company" && !done && (
            <form id="company-form" onSubmit={submitCompany} className="px-5 py-5 space-y-5">
              <Field label="Назив на компанијата *" icon={<Building2 size={14} />} placeholder="ДООЕЛ / АД Пример"
                value={companyForm.company} onChange={(v) => setCompanyForm((f) => ({ ...f, company: v }))} required />
              <Field label="Контакт лице" icon={<User size={14} />} placeholder="Вашето Ime и Презиме"
                value={companyForm.contact} onChange={(v) => setCompanyForm((f) => ({ ...f, contact: v }))} />
              <Field label="Е-пошта *" icon={<Mail size={14} />} type="email" placeholder="вашата@епошта.com"
                value={companyForm.email} onChange={(v) => setCompanyForm((f) => ({ ...f, email: v }))} required />
              <Field label="Телефон" icon={<Phone size={14} />} type="tel" placeholder="+389 2 XXX XXX"
                value={companyForm.phone} onChange={(v) => setCompanyForm((f) => ({ ...f, phone: v }))} />
                ...
                <div className="space-y-2">
                  {COMPANY_TIERS.map((tier) => (
                    <button
                      key={tier.value}
                      type="button"
                      onClick={() => setCompanyForm((f) => ({ ...f, tier: tier.value }))}
                      className={cn(
                        "flex w-full items-start gap-3 rounded-xl border px-4 py-3 text-left transition-colors",
                        companyForm.tier === tier.value
                          ? "border-primary bg-primary-light"
                          : "border-zinc-200 bg-zinc-50 hover:border-zinc-300",
                      )}>
                      <span className={cn(
              ...
              <div>
                ...
                <textarea rows={3} placeholder="Идеи за соработка, прашања…"
            ...
            <div className="flex flex-col items-center justify-center gap-4 px-5 py-14 text-center">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-zinc-500 leading-relaxed max-w-xs">
                  Вашата порака е испратена. Ќе ве контактираме наскоро.
                </p>
              </div>
        ...
        {(step === "member" || step === "company") && !done && (
          <div className="shrink-0 border-t border-zinc-100 px-5 py-3.5 flex gap-3">
            <button type="button" onClick={back}
              className="rounded-xl border border-zinc-200 px-4 py-2.5 text-sm font-semibold text-zinc-600 hover:bg-zinc-50 transition-colors">
              Назад
            </button>
            <button
              type="submit"
              form={step === "member" ? "member-form" : "company-form"}
              disabled={submitting}
...
function ChoiceCard({ icon, title, desc, onClick }: {
  ...
  return (
    <button onClick={onClick}
      className="group flex w-full items-start gap-4 rounded-2xl bg-zinc-100 p-5 text-left transition-colors hover:bg-zinc-200">
      <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-white text-zinc-500 shadow-sm group-hover:shadow transition-shadow">
        {icon}
      </span>
      <div>
