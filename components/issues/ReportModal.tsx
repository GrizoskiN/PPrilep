...
const CATEGORIES = ['road', 'water', 'power', 'garbage', 'park', 'other'] as const
...
const CATEGORY_MK: Record<string, string> = {
  road: 'Патишта', water: 'Вода', power: 'Струја',
  garbage: 'Ѓубре', park: 'Парк', other: 'Друго',
}
...
export default function ReportModal({ userId, onClose, onSuccess }: Props) {
  ...
  async function onSubmit(values: Fields) {
    if (!userId) { toast.error('Мора да сте најавени'); return }

    let photoUrl: string | null = null
    if (file) {
      const ext = file.name.split('.').pop()
      const path = `${crypto.randomUUID()}.${ext}`
      const { data, error } = await supabase.storage.from('issue-photos').upload(path, file, { contentType: file.type })
      if (error) { toast.error('Грешка при прикачување на фотографијата'); return }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-4 border-b border-zinc-200 sticky top-0 bg-white z-10">
          ...
          <button onClick={onClose} className="text-zinc-400 hover:text-zinc-700 cursor-pointer"><X size={16} /></button>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-4 space-y-4">
          ...
          <div>
            <label className="text-xs font-medium text-zinc-700">Улица / локација</label>
            <p className="text-[10px] text-zinc-400 mb-1">Напишете за автоматско пребарување на OpenStreetMap</p>
            <Controller
              name="street_name"
              control={control}
              render={({ field }) => (
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select {...register('district')} className="mt-1 w-full border border-zinc-200 rounded-lg px-3 py-2 text-sm bg-white cursor-pointer focus:border-teal-500 outline-none">
            ...
            <div>
              ...
              <select {...register('category')} className="mt-1 w-full border border-zinc-200 rounded-lg px-3 py-2 text-sm bg-white cursor-pointer focus:border-teal-500 outline-none">
                {CATEGORIES.map(c => <option key={c} value={c}>{CATEGORY_MK[c]}</option>)}
              </select>
            </div>
          </div>

          <div>
            <label className="text-xs font-medium text-zinc-700">Фотографија <span className="text-zinc-400">(незадолжително)</span></label>
            ...
            {preview && (
              ...
              <img src={preview} alt="Преглед" className="mt-2 rounded-lg w-full max-h-32 object-cover border border-zinc-200" />
            )}
          </div>
