...
import ClubSubmitForm from "./ClubSubmitForm";
...
/**
 * Popup shell for the "Пријави клуб" wizard. Bottom sheet on mobile, centered
 * card on desktop. Mirrors SubmitEventModal verbatim — same drag-to-dismiss,
 * Escape handling and enter animation — so the two submission flows feel alike.
 */
export default function SubmitClubModal({ onClose }: Props) {
        ...
        <div
          ...
          <div className="flex shrink-0 items-center justify-between border-b border-zinc-200 bg-white px-4 py-3">
            <div className="flex items-center gap-2">
              <span className="flex h-7 w-7 items-center justify-center rounded-full bg-teal-50 text-base">
                🏅
              </span>
              <h2 className="text-base font-semibold text-zinc-900">Пријави клуб</h2>
            </div>
            <button
              type="button"
              onClick={handleClose}
          ...
          <div className="min-h-0 flex-1">
            <ClubSubmitForm onCancel={handleClose} onClose={handleClose} />
          </div>
        </div>
      </div>
    </>
